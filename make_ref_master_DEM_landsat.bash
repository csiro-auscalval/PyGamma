...
display_usage() {
    ...
    echo "*         <ext image>   name of external reference image (eg. landsat.tif)    *"
    ...
    echo "*******************************************************************************"
    echo -e "Usage: make_ref_master_DEM.bash [proc_file] [rlks] [alks] [multi-look] [subset] <ext image> <roff> <rlines> <azoff> <azlines> <beam>"
    }
...
lsat=$dem_dir/$mli_name"_lsat"
lsat_dem_par=$lsat.dem_par
lsat_trans=$dem_dir/$mli_name"_lsat.eqa_sar"
lsat_float=$dem_dir/$mli_name"_lsat.eqa_sar.float"
lsat_rdc_sim_sar=$dem_dir/$mli_name"_lsat_rdc.sim"
lsat_rdc_dem=$dem_dir/$mli_name"_lsat_rdc.dem"
lsat_rdc_sar=$dem_dir/$mli_name"_lsat_rdc.sim2"
...
if [ -f $image ]; then
    GM geocode $lt_rough $lsat_float $dem_width $lsat_rdc_sim_sar $master_mli_width $master_mli_length 1 0 - - 2 4 -
else
...
if [ -f $image ]; then
    GM init_offsetm $master_mli $lsat_rdc_sim_sar $diff $rlks $alks - - - - $dem_snr - 1 
else
    GM init_offsetm $master_mli $rdc_sim_sar $diff $rlks $alks - - - - $dem_snr - 1 
fi

if [ -f $image ]; then
    GM offset_pwrm $master_mli $lsat_rdc_sim_sar $diff $offs $snr - - $offsets 1 - - -
else
...
if [ -f $image ]; then
    GM offset_pwrm $master_mli $lsat_rdc_sim_sar $diff $offs $snr $rwin $azwin $offsets 2 $nr $naz -
else
...
if [ -f $image ]; then
    GM geocode $lt_fine $lsat_float $dem_width $lsat_rdc_dem $master_mli_width $master_mli_length 1 0 - - 2 4 -
else
