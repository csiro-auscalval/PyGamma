...
display_usage() {
    echo ""
    echo "*******************************************************************************"
    echo "* plot_unwrapped_flag_files:  Copy unwrapped flag files from each             *"
    echo "*                             interferogram directory to a central location   *"
    echo "*                             and check processing results.                   *"
    echo "*                                                                             *"
    echo "*                    Note: for branch-cut unwrapping method only              *"
    echo "*                                                                             *"
    ...
    echo "*******************************************************************************"
    echo -e "Usage: plot_unwrapped_flag_files.bash [proc_file] [list_type]"
    }
...
if [ $list_type -eq 1 ]; then
    ...
    echo " "
    echo "Creating plots for unwrapped flag files..."
else
...
flag_dir=$proj_dir/$track_dir/unw_flag_files
png_dir=$proj_dir/$track_dir/unw_flag_files/png_images


## Copy unwrapped flag files to central directory
while read list; do
    if [ ! -z $list ]; then
	mas=`echo $list | awk 'BEGIN {FS=","} ; {print $1}'`
	slv=`echo $list | awk 'BEGIN {FS=","} ; {print $2}'`
	ifm_dir=$int_dir/$mas-$slv
	file=$mas-$slv"_"$polar"_"$ifm_looks"rlks_filt_cc.flag"

	png=$mas-$slv"_"$polar"_"$ifm_looks"rlks_filt_cc_flag.png"

	cp $ifm_dir/$file $flag_dir
	cp $ifm_dir/ifg.rsc $flag_dir
...
$project $sensor $track_dir $polar $ifm_looks rlks Filtered Interferograms
EOF
...
pdf=$project"_"$sensor"_"$track_dir"_"$polar"_"$ifm_looks"rlks_filt_cc_flag"
...
echo " "
echo "Plots created for filtered interferograms."
echo " "
