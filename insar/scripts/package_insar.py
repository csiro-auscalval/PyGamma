...
def _slc_files(
    ...
    if not isinstance(burst_data, pd.DataFrame):
        df = pd.read_csv(Path(burst_data).as_posix())

    df["acquisition_datetime"] = pd.to_datetime(df["acquistion_datetime"])
    df["date"] = df["acquisition_datetime"].apply(lambda x: pd.Timestamp(x).date())
    df_subset = df[df["date"] == acquisition_date]

    return [item for item in df_subset.url.unique()]
