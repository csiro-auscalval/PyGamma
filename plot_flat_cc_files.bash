...
display_usage() {
    echo ""
    echo "*******************************************************************************"
    echo "* plot_flat_cc_files:  Copy coherence files from each interferogram directory *"
    ...
    echo "*******************************************************************************"
    echo -e "Usage: plot_flat_cc_files.bash [proc_file] [list_type] [rlks] [alks] <beam>"
    }
...
if [ $list_type -eq 1 ]; then
    ...
    echo " "
    echo "Creating plots for flattened coherence files..."
else
    ifm_list=$proj_dir/$track_dir/lists/`grep List_of_add_ifms= $proc_file | cut -d "=" -f 2`
    echo " "
    echo "Creating plots for additional flattened coherence files..."
fi


png_dir=$proj_dir/$track_dir/png_images/png_flat_cc_files
pyrate_dir=$proj_dir/$track_dir/pyrate_files/flat_cc_files
...
if [ -z $beam ]; then #no beam
	...
	if [ ! -z $list ]; then
	    ...
	    file=$mas-$slv"_"$polar"_"$ifm_looks"rlks_flat_utm.cc"
	    png=$mas-$slv"_"$polar"_"$ifm_looks"rlks_flat_utm_cc.png"
...
ls *.cc > flat_cc_list
	...
	if [ ! -z $list ]; then
	    ...
	    slv=`echo $list | awk 'BEGIN {FS=","} ; {print $2}'`
	    ifm_dir=$int_dir/$mas-$slv
	    file=$mas-$slv"_"$polar"_"$beam"_"$ifm_looks"rlks_flat_utm.cc"
	    png=$mas-$slv"_"$polar"_"$beam"_"$ifm_looks"rlks_flat_utm_cc.png"
	    cp $ifm_dir/$file $pyrate_dir
	    cp $ifm_dir/ifg.rsc $pyrate_dir
	    cp $ifm_dir/$png $png_dir
	fi
...
$project $sensor $track_dir $polar $ifm_looks rlks Flattened Coherence
EOF
...
if [ -z $beam ]; then
    pdf=$project"_"$sensor"_"$track_dir"_"$polar"_"$ifm_looks"rlks_flat_cc"
else
    pdf=$project"_"$sensor"_"$track_dir"_"$polar"_"$beam"_"$ifm_looks"rlks_flat_cc"
fi
...
echo " "
echo "Plots created for flattened coherence files."
echo " "
