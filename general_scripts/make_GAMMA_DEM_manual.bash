...
display_usage() {
    echo ""
    echo "*******************************************************************************"
    echo "* make_GAMMA_DEM_manual: Script takes an ASCII file created in ArcGIS         *"
    echo "*                        and creates a DEM and par file for use with GAMMA.   *"
    echo "*                                                                             *"
    echo "*   To be used when AOI is not in Australia. Automatic DEM creation exists    *"
    echo "*   for scenes covering Australia (ie. Australia Sentinel-1 coverage extent)  *"
    echo "*                                                                             *"
    ...
    echo "*******************************************************************************"
    echo -e "Usage: make_GAMMA_DEM_manual.bash [proc_file] [dem_ascii_file] <type> <utm_zone> <ext_ascii> <west> <east> <south> <north>"
    }
