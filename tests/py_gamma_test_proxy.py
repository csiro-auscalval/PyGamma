...
class PyGammaTestProxy(object):
    ...
    def phase_sim(self, SLC1_par: str, OFF_par: str, baseline: str, hgt, sim_unw: str, ph_flag = None, bflag = None, definition = None, delta_t = None, int_mode = None, SLC2R_par: str = None, ph_mode = None):
        ...
        valid_values = [0, 1] + [None]
        result = self._validate("phase_sim", int_mode == "-" or int_mode in valid_values, result, f"int_mode is not a valid value (expects: {valid_values}, got: {int_mode})")
        if SLC2R_par is not None:
    ...
    def coord_to_sarpix(self, SLC_par: str, OFF_par: str, DEM_par: str, north_lat, east_lon, hgt, DIFF_par = None):
        ...
        if SLC_par is not None:
            result = self._validate("coord_to_sarpix", SLC_par == "-" or Path(SLC_par).exists(), result, f"SLC_par path does not exist ({SLC_par})")
        if OFF_par is not None:
    ...
    def phase_sim_orb(self, SLC1_par: str, SLC2R_par: str, OFF_par: str, hgt, sim_unw: str, SLC_ref_par: str = None, definition = None, delta_t = None, int_mode = None, ph_mode = None):
        ...
        valid_values = [0, 1] + [None]
        result = self._validate("phase_sim_orb", int_mode == "-" or int_mode in valid_values, result, f"int_mode is not a valid value (expects: {valid_values}, got: {int_mode})")
        valid_values = [0, 1] + [None]
    ...
    def dh_map_orb(self, SLC1_par: str, SLC2R_par: str, OFF_par: str, hgt, dp, dpdh: str, dh: str, SLC_ref_par: str = None, int_mode = None):
        ...
        valid_values = [0, 1] + [None]
        result = self._validate("dh_map_orb", int_mode == "-" or int_mode in valid_values, result, f"int_mode is not a valid value (expects: {valid_values}, got: {int_mode})")
        if self._wraps is not None:
    ...
    def dop_interf(self, SAR_par1: str, PROC_par1: str, PROC_par2: str, PROC_par1_out: str, PROC_par2_out: str, dop: str):
        ...
        if SAR_par1 is not None:
            result = self._validate("dop_interf", SAR_par1 == "-" or Path(SAR_par1).exists(), result, f"SAR_par1 path does not exist ({SAR_par1})")
        if PROC_par1 is not None:
            result = self._validate("dop_interf", PROC_par1 == "-" or Path(PROC_par1).exists(), result, f"PROC_par1 path does not exist ({PROC_par1})")
        if PROC_par2 is not None:
            result = self._validate("dop_interf", PROC_par2 == "-" or Path(PROC_par2).exists(), result, f"PROC_par2 path does not exist ({PROC_par2})")
    ...
    def PALSAR_antpat(self, SAR_par: str, PROC_par: str, PAL_ANT: str, ant_file: str, Beam_ID = None, TX_POL = None, RX_POL = None, Beam = None):
        ...
        self.call_count["PALSAR_antpat"] += 1

        if SAR_par is not None:
            result = self._validate("PALSAR_antpat", SAR_par == "-" or Path(SAR_par).exists(), result, f"SAR_par path does not exist ({SAR_par})")
        if PROC_par is not None:
            result = self._validate("PALSAR_antpat", PROC_par == "-" or Path(PROC_par).exists(), result, f"PROC_par path does not exist ({PROC_par})")
        if PAL_ANT is not None:
            result = self._validate("PALSAR_antpat", PAL_ANT == "-" or Path(PAL_ANT).exists(), result, f"PAL_ANT path does not exist ({PAL_ANT})")
        if ant_file is not None and ant_file != "-":
            Path(ant_file).touch()
        valid_values = [0, 1] + [None]
        result = self._validate("PALSAR_antpat", TX_POL == "-" or TX_POL in valid_values, result, f"TX_POL is not a valid value (expects: {valid_values}, got: {TX_POL})")
        valid_values = [0, 1] + [None]
        result = self._validate("PALSAR_antpat", RX_POL == "-" or RX_POL in valid_values, result, f"RX_POL is not a valid value (expects: {valid_values}, got: {RX_POL})")
        if self._wraps is not None:
            result = self._wraps.PALSAR_antpat(*supplied_args)

        self.call_sequence.append(PyGammaCall("MSP", "PALSAR_antpat", supplied_args, result[0]))
        self._on_error("PALSAR_antpat", supplied_args, result[0])
        return result
    ...
    def uchar2float(self, infile: str, outfile, scale, exp, offset = None):
        ...
        if infile is not None:
            result = self._validate("uchar2float", infile == "-" or Path(infile).exists(), result, f"infile path does not exist ({infile})")
        if self._wraps is not None:
    ...
    def par_ASF_SLC(self, CEOS_SAR_leader: str, SLC_par: str, CEOS_data: str = None, SLC: str = None):
        ...
        if CEOS_SAR_leader is not None:
            result = self._validate("par_ASF_SLC", CEOS_SAR_leader == "-" or Path(CEOS_SAR_leader).exists(), result, f"CEOS_SAR_leader path does not exist ({CEOS_SAR_leader})")
    ...
    def SLC_interp_map(self, SLC_2: str, SLC1_par: str, SLC2_par: str, OFF_par: str, SLC_2R: str, SLC2R_par: str, OFF_par2: str, coffs_sm: str, loff = None, nlines = None, mode = None):
        ...
        if coffs_sm is not None:
            result = self._validate("SLC_interp_map", coffs_sm == "-" or Path(coffs_sm).exists(), result, f"coffs_sm path does not exist ({coffs_sm})")
        valid_values = [0, 1] + [None]
    ...
    def ScanSAR_burst_to_mosaic(self, DATA_tab: str, mosaic: str, MLI_par: str, mflg = None, dtype: str = None):
        ...
        if dtype is not None:
            result = self._validate("ScanSAR_burst_to_mosaic", dtype == "-" or Path(dtype).exists(), result, f"dtype path does not exist ({dtype})")
        if self._wraps is not None:
    ...
    def par_TX_GRD(self, annotation_XML: str, COSAR: str, GRD_par, GRD: str, pol = None):
        ...
        if COSAR is not None:
            result = self._validate("par_TX_GRD", COSAR == "-" or Path(COSAR).exists(), result, f"COSAR path does not exist ({COSAR})")
        if GRD is not None and GRD != "-":
            Path(GRD).touch()
        if self._wraps is not None:
