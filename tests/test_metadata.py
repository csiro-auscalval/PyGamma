...
def test_slc_inputs(query_results):
    ...
    for pol in ["VV", "VH"]:
        ...
        assert(set(slc_test.acquistion_datetime.values) == set(slc_val.acquistion_datetime.values))
        for swath in ["IW1", "IW2", "IW3"]:
            slc_test_swath = slc_test[slc_test.swath == swath]
            slc_val_swath = slc_val[slc_val.swath == swath]
            assert (set(sum(slc_test_swath.burst_number.values, [])) == set(sum(slc_val_swath.burst_number.values, [])))
...
def test_s1datadownload(tmp_path, query_results):
    ...
    slc_inputs_df = pd.concat(
        [slc_inputs(results_test[pol]) for pol in ["VV", "VH"]]
    )
    download_list = slc_inputs_df.url.unique()
    raw_dir = tmp_path.joinpath("RAW")
    for fp in download_list:
        fp = Path(fp)
        if fp.stem != "S1A_IW_SLC__1SDV_20160101T214934_20160101T215000_009306_00D719_38CC":
            continue
        ...
        test_files = [item.name for item in raw_dir.joinpath(fp.stem).with_suffix(".SAFE").glob("**/*")]
...
def test_creategammadem(tmp_path):
    ...
    assert outdir.joinpath(f"{track_frame}.dem.par").exists()
    assert outdir.joinpath(f"{track_frame}.dem").exists()

    with open(outdir.joinpath(f"{track_frame}.dem.par").as_posix(), "r") as fid:
        test_pars = [item.rstrip().split(':')[1].strip() for item in fid.readlines() if ':' in item]
        with open(__DATA__.joinpath(f"{track_frame}.dem.par").as_posix(), "r") as fid1:
            val_pars = [item.strip().split(':')[1].strip() for item in fid1.readlines() if ':' in item]
