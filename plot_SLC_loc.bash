...
display_usage() {
    echo ""
    echo "*******************************************************************************"
    echo "* plot_SLC_loc:  Creates a location map image of SLC extent using GMT.        *"
    echo "*                                                                             *"
    echo "* input:  [proc_file]  name of GAMMA proc file (eg. gamma.proc)               *"
    echo "*         [scene]      scene ID   (e.g. 20070112)                             *"
    echo "*         [msp_par]    SLC MSP parameter file                                 *"
    echo "*         [sensor]     PALSAR/ERS/ASAR/TSX/RSAT2/CSK                          *"
    echo "*         [ras_image]  MLI image (.ras)                                       *"
    echo "*                                                                             *"
    echo "* author: Sarah Lawrie @ GA       16/07/2014, v1.0                            *"
    echo "*******************************************************************************"
    echo -e "Usage: plot_SLC_loc.bash [proc_file] [scene] [msp_par] [sensor] [ras_image]"
    }
...
if [ $sensor == ERS ]; then
    sensor_name=`grep ERS_sensor= $proc_file | cut -d "=" -f 2`
else
    sensor_name=$sensor
fi
...
if [ ! -e $msp_par ]; then
  echo "ERROR: SLC parameter file does not exist!"
  exit 1
fi
