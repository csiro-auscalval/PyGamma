...
display_usage() {
    echo ""
    echo "*******************************************************************************"
    echo "* plot_filt_ifms:  Copy filtered ifm files from each interferogram directory  *"
    ...
    echo "*******************************************************************************"
    echo -e "Usage: plot_filt_ifms.bash [proc_file] [list_type] [rlks] [alks] <beam>"
    }
...
if [ $list_type -eq 1 ]; then
    ...
    echo " "
    echo "Creating plots for filtered interferograms..."
else
    ifm_list=$proj_dir/$track_dir/lists/`grep List_of_add_ifms= $proc_file | cut -d "=" -f 2`
    echo " "
    echo "Creating plots for additional filtered interferograms..."
fi


png_dir=$proj_dir/$track_dir/png_images/png_filt_ifms
pyrate_dir=$proj_dir/$track_dir/pyrate_files/filt_ifms
...
if [ -z $beam ]; then
    pdf=$project"_"$sensor"_"$track_dir"_"$polar"_"$ifm_looks"rlks_filt_ifms"
else
    pdf=$project"_"$sensor"_"$track_dir"_"$polar"_"$beam"_"$ifm_looks"rlks_filt_ifms"
fi
