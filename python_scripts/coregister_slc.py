...
class CoregisterSlc:
    def __init__(
        ...
        if not self.r_dem_master_mli_par.exists():
            _LOG.error(FileNotFoundError(f"{self.r_dem_master_mli_par.as_posix()} does not exists"))

        self.r_dem_master_slc_par = self.slc_master.with_suffix(".slc.par")
        if not self.r_dem_master_slc_par.exists():
            _LOG.error(FileNotFoundError(f"{self.r_dem_master_slc_par.as_posix()} does not exists"))

        self.slc_slave_par = self.slc_slave.with_suffix(".slc.par")
        if not self.slc_slave_par.exists():
            _LOG.error(FileNotFoundError(f"{self.slc_slave_par.as_posix()} does not exists"))

        self.slave_mli_par = self.slave_mli.with_suffix(".mli.par")
        if not self.slave_mli_par.exists():
            _LOG.error(FileNotFoundError(f"{self.slave_mli_par.as_posix()} does not exist"))
    ...
    def set_tab_files(self, out_dir: Optional[Union[Path, str]] = None):
        ...
        self._write_tabs(self.r_slave_slc_tab, f"r{self.slc_slave.stem}", self.slc_slave.parent)

        # write master slc tab file
        self.master_slc_tab = out_dir.joinpath("master_slc_tab")
        self._write_tabs(self.master_slc_tab, self.slc_master.stem, self.slc_master.parent)
    ...
    def reduce_offset(self, range_offset_max: Optional[int] = 64, azimuth_offset_max: Optional[int] = 32):
        """Reduce offset estimation to max offset values"""
        self.range_step = int((self.master_sample.slc_width_end - self.master_sample.slc_width_start) / 64)
        if range_offset_max > self.range_step:
            self.range_step = range_offset_max
        self.azimuth_step = int((self.master_sample.slc_lines_end - self.master_sample.slc_lines_start) / 64)
        if azimuth_offset_max > self.azimuth_step:
            self.azimuth_step = azimuth_offset_max

    def _write_tabs(self, tab_file: Union[Path, str], _id: str, data_dir: Optional[Union[Path, str]] = None):
    ...
    @staticmethod
    def _grep_offset_parameter(offset_file: Union[Path, str], match_start_string: Optional[str] = None):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            while abs(d_azimuth) > max_azimuth_threshold and iteration < max_iteration:
                ...
                command = [
                    ...
                    "0.2",
                    str(self.range_step),
                    str(self.azimuth_step),
                    str(self.master_sample.slc_width_start),
                    str(self.master_sample.slc_width_end),
                    str(self.master_sample.slc_lines_start),
                    str(self.master_sample.slc_lines_end)
                ]
                run_command(command, temp_dir.as_posix())
                ...
                _std_output = run_command(command, temp_dir.as_posix(), return_stdout=True)
