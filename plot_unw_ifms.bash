...
display_usage() {
    echo ""
    echo "*******************************************************************************"
    echo "* plot_unw_ifms:  Copy unw files from each interferogram directory to a       *"
    ...
    echo "*******************************************************************************"
    echo -e "Usage: plot_unw_ifms.bash [proc_file] [list_type] [rlks] [alks] <beam>"
    }
...
png_dir=$proj_dir/$track_dir/png_images/png_unw_ifms
pyrate_dir=$proj_dir/$track_dir/pyrate_files/unw_ifms
...
if [ -z $beam ]; then #no beam
	...
	if [ ! -z $list ]; then
	    ...
	    file=$mas-$slv"_"$polar"_"$ifm_looks"rlks_eqa.unw"
	    png=$mas-$slv"_"$polar"_"$ifm_looks"rlks_eqa_unw.png"
...
ls *.unw > unw_list
	...
	if [ ! -z $list ]; then
	    ...
	    slv=`echo $list | awk 'BEGIN {FS=","} ; {print $2}'`
	    ifm_dir=$int_dir/$mas-$slv
	    file=$mas-$slv"_"$polar"_"$beam"_"$ifm_looks"rlks_eqa.unw"
	    png=$mas-$slv"_"$polar"_"$beam"_"$ifm_looks"rlks_eqa_unw.png"
	    cp $ifm_dir/$file $pyrate_dir
	    cp $ifm_dir/ifg.rsc $pyrate_dir
	    cp $ifm_dir/$png $png_dir
	fi
...
if [ -z $beam ]; then
    pdf=$project"_"$sensor"_"$track_dir"_"$polar"_"$ifm_looks"rlks_unw_ifms"
else
    pdf=$project"_"$sensor"_"$track_dir"_"$polar"_"$beam"_"$ifm_looks"rlks_unw_ifms"
fi
