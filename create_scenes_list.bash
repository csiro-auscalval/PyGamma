...
## Create additional scene list
    ...
    if [ -f $add_s1_file_list ]; then # Sentinel-1 data
	cp $scene_list temp1
	sort temp1 > scenes.list_org
	rm -f temp1
	awk '/FILES_TO_DOWNLOAD/ { show=1 } show; /SUBSET_BURSTS/ { show=0 }' $add_s1_file_list | tail -n+3 | head -n -2 > file_list
	awk '{print $1}' file_list | sort | uniq > $add_scene_list
	rm -rf file_list $scene_list
	cat  scenes.list_org >> $scene_list
	cat $add_scene_list >> $scene_list

    else # other sensors
	cp $scene_list temp1
	sort temp1 > scenes.list_org
	rm -f temp1
	comm -13 scenes.list_org $scene_list > $add_scene_list
    fi
