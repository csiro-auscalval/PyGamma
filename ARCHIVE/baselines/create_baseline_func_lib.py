"""
Library of functions for baseline calculations
...
- calc_orbit_pos: calculate satellite orbit position from orbital state
                  vectors
...
def epoch_baselines(epochs, bperp, masidx, slvidx, supermaster):
    ...
    # Count number of ifms and epochs
    nifms = len(bperp)
    nepochs = len(epochs)
    print(nifms, "interferograms and", nepochs, "epochs in the network.")

    # Initialise design matrix 'A'
    A = np.zeros((nifms+1,nepochs))
